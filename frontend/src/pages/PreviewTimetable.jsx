...
export default function PreviewTimetable() {
  ...
  const divisions = ["D1", "D2", "D3", "D4", "D5", "D6", "D7", "D8", "D8", "D9", "D10", "D11", "D12"];
  ...
  const fetchAllDivisionsTimetable = async () => {
    ...
    try {
        ...
        try {
          ...
          days.forEach(day => {
            ...
            timeSlots.forEach(slot => {
              formattedSchedule[day][slot.value] = fetchedSchedule[day]?.[slot.value] || "free";
      ...
      if (loadedCount === 0) {
        ...
        setSuccessMsg(`Loaded ${loadedCount} division timetables for ${branch} - Semester ${sem}`);
  ...
  const exportAllToText = () => {
    ...
    Object.entries(allSchedules).forEach(([division, schedule]) => {
      ...
      days.forEach(day => {
        ...
        timeSlots.forEach(slot => {
          const faculty = schedule[day][slot.value];
          content += `  ${slot.label}: ${faculty === "free" ? "Free Slot" : faculty}\n`;
        });
        content += "\n";
      });
    ...
    a.href = url;
    a.download = `${branch}_Sem${sem}_All_Divisions_Timetable_${new Date().toISOString().split('T')[0]}.txt`;
      ...
      <div key={division} className="mb-8 last:mb-0">
        ...
        <div className="flex items-center justify-between mb-4 print:mb-2 pt-5">
          <div className="flex items-center gap-3">
            <div className={`px-4 py-2 rounded-lg ${branchColors[branch] || "bg-gray-100"} print:px-2 print:py-1`}>
            ...
            <div className="overflow-x-auto print:overflow-visible">
              <table className="w-full print:min-w-full print:border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 print:divide-y-0">
                  {timeSlots.map((slot) => {
                    const hasVisibleData = days.some(day => 
                      filteredSchedule && filteredSchedule[day] && filteredSchedule[day][slot.value] !== undefined
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        {days.map((day) => {
                          const faculty = filteredSchedule && filteredSchedule[day] 
                            ? filteredSchedule[day][slot.value] 
                            : schedule[day][slot.value];
                          
                          if (faculty === undefined) return (
                            <td key={day} className="print:p-2 print:border print:border-gray-300"></td>
                          );

                          const facultyColor = facultyColors[faculty] || "bg-gray-100 text-gray-700 border-gray-200";
                          
                          return (
                            <td key={day} className="print:p-2 print:border print:border-gray-300">
                              <div className="p-3 print:p-2">
                                <div className={`px-3 py-2 rounded-lg border text-center transition-all print:p-1 print:rounded-sm ${facultyColor} print:border-0 print:text-xs`}>
                                  {faculty === "free" ? (
      ...
      <div className="relative z-10 max-w-7xl mx-auto print:max-w-none">
        ...
        <div className="print:hidden mb-6">
          <div className="flex items-center text-sm text-gray-600 mb-4">
            ...
            <span className="font-medium text-blue-600">Preview All Timetables</span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  {["CSE", "CSE(AIML)", "DS", "ECE", "EEE", "ME", "CE"].map((b) => (
            ...
            <div className="print:hidden mt-6 bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(facultyColors).map(([faculty, color]) => (
                  <div
                    key={faculty}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium ${color} border`}
                  >
        ...
        <div className="hidden print:block print:mt-8 print:pt-4 print:border-t print:border-gray-300 print:text-center print:text-xs print:text-gray-500">
          <p>This timetable was generated automatically by Timetable Management System</p>
